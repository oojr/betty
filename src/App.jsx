...
const ProjectSelectionModal = () => {
  const { apiKey, projectType, setProjectType, clearHistory, addMessage } =
    useDirectorStore();
  ...
  const projects = [
    {
      ...
      bootstrap:
        "open App.jsx look at the structure then make changes to the slide I need",
    },
    {
      id: "mobile",
      ...
      bootstrap:
        "open the expo project, layout is where the main screen is, try to make the app without any external dependencies just use generic React Native components, also try not to use external images from the web",
    },
    {
      id: "web",
      ...
      bootstrap:
        "Open the read the nextjs project and make changes to it, try not to use external images from the web",
    },
    {
      id: "video",
      title: "Motion Video",
      ...
      bootstrap:
        "Update the remotion project, the Root is where the core video logic lives",
    },
    {
      id: "excel",
      title: "Data Analysis",
      ...
      bootstrap:
        "open App.jsx look at the stucture and create a chart visualization based on the provided data",
    },
    {
      id: "word",
      ...
      bootstrap:
        "open the file App.jsx look at the strucutre and make changes to generateDocument",
    },
  ];
  ...
  const handleSelect = (p) => {
    ...
    setProjectType(p.id);
    // Initialize chat history IMMEDIATELY with bootstrap followed by greeting
    clearHistory(p.greeting, p.bootstrap);
  };
...
const ChatInterface = () => {
  ...
  const handleSend = async () => {
    if (!input.trim() && attachedFiles.length === 0 || !apiKey || isProcessing) return;

    let userMsg = input;

    // Prepend attached file content to the user message
    attachedFiles.forEach((file) => {
      userMsg = `[Attached File: ${file.name}]\n\`\`\`\n${file.content.slice(0, 5000)}\n\`\`\`` + (userMsg ? `\n\n${userMsg}` : '');
    });
      ...
      <div className="p-4 bg-gray-800 border-t border-gray-700">
        {attachedFiles.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-3">
            {attachedFiles.map((file, index) => (
              <span
                key={index}
                className="flex items-center bg-yellow-600/20 text-yellow-200 text-xs px-3 py-1 rounded-full border border-yellow-600/40"
              >
                <span className="mr-2">{file.name}</span>
                <button
                  onClick={() => handleRemoveFile(file.name)}
                  className="text-yellow-200/70 hover:text-yellow-100 ml-1"
        ...
        <div className="flex gap-2 items-end">
          ...
          <textarea
            ...
            disabled={!apiKey || isProcessing || !previewUrl}
            rows={1}
          ...
          >
            {isProcessing || (apiKey && !previewUrl) ? "Processing" : "Run"}
          </button>
        </div>
      </div>
    </div>
